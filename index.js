...
Change log:
	...
	Last Edited -- 02 Nov 2022 -- 23.14 WIB Indonesian Time
...
function validateUser() {
  (async () => {
    try {
      ...
      await atlasData(parseId).then(async data => {
        let result = data
        let id = result.userID
					console.log("used by: ", id);
        reply(txt);
      });
    } catch {
      reply(NotRegistered);
...
`.trim()
			...
			case 'deployment': {
				...
				setTimeout(() => { reply("```Azusa Bot, in version releases of v3.0```") },4000)
			...
			case 'yatta': {
				let user_id = parseInt(m.sender.split('@')[0])
				let id = user_id
					(async () => {
						try {
							await atlasData(id).then(async data => {
								let result = data
								let id = result.userID
			...
			case 'atlastest':{
				try { 
					let txt = "Jadi maseh!"
					validateUser().then(reply(txt))
				} catch { reply("yahh error :(") }
			} break
				...
				try {
					...
					await atlasData(parseId).then(async result => {
						...
						async (error, result) => {
							...
							alpha.send1ButMes(m.chat, txt, `© ${ownername}`, `getuserdata`, `Cek Profilmu`, m)
							//reply(txt);
							console.log(result);
						})
				}
    			...
    			if (error) {
      			return reply(error);
					...
					for (let data of datas) {
						...
						txt += `Umur : ${data.age}\n` 
						txt += `Hobi : ${data.hobby}\n` 
						txt += `Nomor : ${data.userID}\n` 
						txt += `Status Premium : ${data.isPremium}\n` 
						txt += `Serial : ${data.userSerial}\n` 
			...
			case 'biganimetiddies': case 'animebellybutton': case 'hentai4everyone': case 'neko': case 'waifu': case 'loli': case 'milf': {
				...
				let parseId = parseInt(user_id);
					await atlasData(parseId).then(async result => {
						let user = result.userID;
						//
						if (!user == parseId) { return reply(NotRegistered) } else {
							//
							reply(lang.wait())
