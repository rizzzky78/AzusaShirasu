...
//
            ...
            var but = [
				...
				await alpha.send5ButImg(from, `© ${ownername}`,gem, but , { userJid: m.chat, quoted: m })
				}
				break
            case 'pay': case 'bayar':{
...
*/
				...
				/*if(db.data.users[m.sender].limit < 1) return alpha.send2ButMes(m.chat, lang.Nolimit(prefix), `© ${ownername}`, `daily`, `👉 Daily`, `weekly`, `Weekly 👈`, m)*/
				reply(lang.wait())
				axios.get(`https://api.lolhuman.xyz/api/random/${command}?apikey=rizzzuchi78apikey`)
				.then(({data}) => {
				alpha.sendImage(m.chat, data.url, lang.ok(), m)
				})
		/*db.data.users[m.sender].limit -= 1*/				}				
...
case 'randomhentai':
	...
	case 'hentai4everyone':{
		reply(lang.wait())
		axios.get(`https://api.lolhuman.xyz/api/random2/${command}?apikey=rizzzuchi78apikey`)
		.then(({data}) => {
		alpha.sendImage(m.chat, data.url, lang.ok(), m)
		})
/*db.data.users[m.sender].limit -= 1*/	}		
